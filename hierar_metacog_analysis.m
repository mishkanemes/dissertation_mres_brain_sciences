...
for subj = 1:length(subjects)
    ...
    % if code is applied over the entire experiment, n = all trials
    % otherwise n=1:80 for S1; n=81:160 for S2 and n=161:240 for S3
    
    for n = 1:length(trial_be)
    ...
    % discretize continuous confidence ratings in bins of equal intervals
    
    conf_bin = zeros (length(rating), 1);
    
    for i = 1:length(rating)
        if rating(i) >= 0.5 && rating(i) < 0.625
            conf_bin(i) = 1;
        elseif rating(i) >= 0.625 && rating(i) < 0.75
            conf_bin(i) = 2;
        elseif rating(i) >= 0.75 && rating(i) < 0.875
            conf_bin(i) = 3;
...
% % %             conf_bin(i) = 5;
    ...
    nR_S1 = zeros(1,8);  % S1 responses = targetright = 0
    nR_S2 = zeros(1,8);  % S2 responses = targetleft = 1
...
%         mle.type2c_rS2 (subj,:) = fit.t2ca_rS2;
    ...
    save (['hmetad_metacog_analogue_4bins'], 'mle');
